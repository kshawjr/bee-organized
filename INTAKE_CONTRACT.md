...
| `source` | no — **but set it** | Answers *"how did the client hear about us"*. Make MUST set this explicitly per scenario or attribution is lost. Scenario slugs are **normalized server-side** (`lib/lead-source.ts`) into the admin `lead_sources` vocabulary before storage: any `*_assessment` slug → `Website`, `web_form`/`website_form` → `Website`, `facebook_lead_ad` → `Facebook`, `instagram_lead_ad` → `Instagram`. Absent → `Website` (this door is the website form). An **unknown** slug is stored verbatim — it shows up off-vocab on the card, deliberately visible; add it to the map in `lib/lead-source.ts` rather than inventing new slugs casually. Sending the human label directly (e.g. `Referral`) also works — labels pass through unchanged. |
