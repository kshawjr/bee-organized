...
//   DELETE — soft-delete (deleted_at = now). `?purge=1` hard-deletes.
...
export async function GET(
  ...
  return NextResponse.json(mapCompanyRow(data))
}
...
export async function DELETE(
  ...
  if (purge === '1' || purge === 'true') {
    const { error } = await supabaseService.from('companies').delete().eq('id', id)
    if (error) {
      console.error('[companies DELETE purge]', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
