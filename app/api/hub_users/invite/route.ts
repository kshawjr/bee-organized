...
export async function POST(request: NextRequest) {
  ...
  if (!isCorporateTier && location_id) {
    ...
    // reservations so two invites can't be issued against one slot). Seats
    // scheduled for removal are excluded: inviting into one would hand the
    // new member a login that dies on the removal date.
    const { data: seatsAtTier, error: seatsErr } = await supabaseService
      .from('subscription_seats')
      .select('id, user_id, status, scheduled_removal_at')
    ...
    const availableSeats = (seatsAtTier || []).filter(
