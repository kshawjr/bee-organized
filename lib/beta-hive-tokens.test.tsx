...
describe('masthead meta-row — Type-only now (Assigned moved to the corner)', () => {
  it('the meta band carries Type under a hairline top rule, its cell does not grow, and "Assigned to" is gone from the band', async () => {
    stubEngagementFetch(basePayload())
    const { host, unmount } = await mount(
      <EngagementPanel engagementId="eng-1" onClose={() => {}} setToast={() => {}} lookupOptions={{ sources: [], projectTypes: ['Client', 'Move'] } as any} />
    )
    const typeLabel = [...host.querySelectorAll('p')].find(p => (p.textContent || '').trim() === 'Type')!
    expect(typeLabel).toBeTruthy()
    // Assignees left the band for the top-right corner — there is no longer
    // an "Assigned to" label anywhere (the corner is faces-only, no label).
    expect([...host.querySelectorAll('p')].some(p => (p.textContent || '').trim() === 'Assigned to')).toBe(false)
    const typeCell = typeLabel.parentElement as HTMLElement
    const row = typeCell.parentElement as HTMLElement
    // Still a defined meta band — hairline top rule, content-sized cell (no
    // phantom stretch column).
    expect(row.style.borderTop).toBe(T.border.divider)
    expect(typeCell.style.flex).not.toContain('220')
    expect(typeCell.style.flexGrow === '1').toBe(false)
    await unmount()
  })

  it('the assignee corner rides the masthead top-right — unassigned shows the click-to-assign ghost affordance', async () => {
    stubEngagementFetch(basePayload())
    const { host, unmount } = await mount(
      <EngagementPanel engagementId="eng-1" onClose={() => {}} setToast={() => {}} lookupOptions={{ sources: [], projectTypes: ['Client', 'Move'] } as any} />
    )
    // No assignees in the payload → the corner is the ghost, and it invites
    // a click (it is a button, not a blank corner).
    const trigger = host.querySelector('button[aria-label="Assign a team member"]') as HTMLElement
